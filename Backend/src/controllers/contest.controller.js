...
export const createContest = asyncHandler(async (req, res) => {
  const { name, description, startTime, endTime, visibility, password } =
    req.body;
  ...
  if (visibility === "PUBLIC" && req.user.role !== "ADMIN") {
    throw new ApiError(
      403,
      "Only admins can create Public contests. You can only create Private contests.",
    );
  }
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, createdContest, "Contest created successfully"));
});
...
export const registerForContest = asyncHandler(async (req, res) => {
  ...
  if (contest.visibility === "PRIVATE") {
    if (!password) {
      throw new ApiError(
        400,
        "This is a private contest. Password is required.",
      );
    }
...
export const getContest = asyncHandler(async (req, res) => {
  ...
  const isAuthor = userId
    ? contest.author._id.toString() === userId.toString()
    : false;
  ...
  if (contest.visibility === "PRIVATE" && !isAuthor) {
    ...
    if (!isRegistered) {
      throw new ApiError(
        403,
        "You must register for this private contest first.",
      );
    }
  }
...
export const removeProblemFromContest = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, contest, "Problem removed from contest"));
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        201,
        { contest, problem },
        "Problem securely created and linked to contest",
      ),
    );
});
